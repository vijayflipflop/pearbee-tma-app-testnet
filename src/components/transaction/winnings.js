...
const WinningsTransactions = () => {
  ...
  const winningTranasctions = async () => {
    try {
      const resp = await services.get(WITHDRAW_TRANSACTIONS);
  ...
  const fetchWinningsTranactions = async () => {
    try {
      const resp = await services.get(`${WITHDRAW_TRANSACTIONS}?page=${page}`);
      const {
...
export default WinningsTransactions;
